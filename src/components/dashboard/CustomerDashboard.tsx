  ...
  const fetchProducts = async () => {
    ...
    if (!error && data) {
      ...
      const productsWithLocation = active.map((p: any) => {
        ...
        const farmLat = farm?.lat ?? null;
        const farmLng = farm?.lng ?? null;
        let deliveryDistance: number | null = null;
        let deliveryCharge: number | null = null;

        if (farmLat && farmLng && savedCoords) {
          const dist = calculateDistance(farmLat, farmLng, savedCoords.lat, savedCoords.lon);
                        ...
                        {product.delivery_charge !== null && product.delivery_charge !== undefined ? (
                          <div className="flex items-center gap-1.5 text-xs">
                            ...
                            <span className="text-muted-foreground">
                              Delivery: ₹{product.delivery_charge} ({product.delivery_distance} km)
                            </span>
                          </div>
                        ) : customerCoords ? (
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            <Truck className="w-3 h-3" />
                            <span>Farm location not set</span>
                          </div>
                        ) : null}
